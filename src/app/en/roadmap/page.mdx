...
**Status: COMPLETE** — Added `list_agent_sessions()`, `create_session_with_label()`, `switch_agent_session()` to kernel. API: `GET/POST /api/agents/&lbrace;id&rbrace;/sessions`, `POST /api/agents/&lbrace;id&rbrace;/sessions/{sid}/switch`. UI: session dropdown in chat header with badge count, new session button, click-to-switch, active session indicator.
...
3. API: `/api/agents/&lbrace;id&rbrace;/sessions` list, `/api/agents/&lbrace;id&rbrace;/sessions/{label}` CRUD
