...
LibreFang provides a [`justfile`](https://github.com/casey/just) for common development commands:
...
- Run a specific crate's tests: `cargo test -p librefang-kernel`
- Run a single test: `cargo test -p librefang-kernel test_my_feature`
- Integration tests in `tests/` directory have access to the crate's public API only
